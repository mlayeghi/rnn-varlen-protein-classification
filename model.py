...
def rnn_model(data, seqs_len, args):
    '''
    ...
    with tf.variable_scope("rnn",
        ...
        if args.rnn_type == 'basiclstm':
            ...
            _, states = tf.nn.dynamic_rnn(multi_cells, data, dtype=tf.float32, sequence_length=seqs_len)
            ...
            lstm_cells = [tf.contrib.rnn.LSTMCell(num_units=args.hidden_dim, use_peepholes=True)
                          for layer in range(args.n_layers)]
            drop_cells = [tf.contrib.rnn.DropoutWrapper(cell, input_keep_prob=args.in_keep_prob)
